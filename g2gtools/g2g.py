...
def log_stats(stats, chromosome=None):
    if not chromosome:
        LOG.info("TOTAL STATISTICS")
        for s, stat in stats['TOTAL'].iteritems():
            LOG.info("{0:,}\t\t{1}".format(stat, s))
    else:
        LOG.info("CHROMSOME {0} STATISTICS".format(chromosome))
        for s, stat in stats[chromosome].iteritems():
            LOG.info("{0:,}\t\t{1}".format(stat, s))


def update_stats(stats, chromosome, reason):

    if chromosome not in stats:
        stats[chromosome] = {}

    if 'TOTAL' not in stats:
        stats['TOTAL'] = {}

    if reason in stats[chromosome]:
        stats[chromosome][reason] += 1
    else:
        stats[chromosome][reason] = 1

    if reason in stats['TOTAL']:
        stats['TOTAL'][reason] += 1
    else:
        stats['TOTAL'][reason] = 1
...
def vcf2chain(input_file, fasta_file, strain, output_file, vcf_keep=False, passed=False, quality=False, diploid=False):
    ...
    if output_file:
        if diploid:
            left.out = open("{0}.left".format(output_file), 'w')
            right.out = open("{0}.right".format(output_file), 'w')
            chain_info['left'] = left
            chain_info['right'] = right
        else:
            left.out = open(output_file, 'w')
            right.out = open(os.devnull, 'w')
            chain_info['left'] = left
    else:
        if diploid:
            left.out = sys.stdout
            right.out = sys.stdout
            chain_info['left'] = left
            chain_info['right'] = right
        else:
            left.out = sys.stdout
    ...
    for vcf_rec in vcf_file:
        ...
        if counter_lines % 10000 == 0:
            LOG.info("Processed {0:,} lines, {1}".format(counter_lines, format_time(start, time.time())))

        gt = vcf_file.parse_gt(strain)

        LOG.debug('\n')
        LOG.debug(vcf_rec)
        LOG.debug(gt)
        LOG.debug(vcf_rec.SAMPLES[sample_index])

        if passed and 'PASS' not in vcf_rec.FILTER:

            LOG.debug("TOSSED: FILTERED ON PASS")
            stats = update_stats(stats, vcf_rec.CHROM, 'FILTERED ON PASS')

            if vcf_keep:
                vcf_discard.write(vcf_file.current_line)
                vcf_discard.write("\n")
            continue
            ...
            stats = update_stats(stats, vcf_rec.CHROM, 'FILTERED ON QUALITY')
            ...
            stats = update_stats(stats, vcf_rec.CHROM, 'NO STRAIN DATA')
            ...
            stats = update_stats(stats, vcf_rec.CHROM, 'HETEROZYGOUS')
        ...
        for ci, lr in chain_info.iteritems():
            ...
            if gt.ref == alt_seq:
                ...
                stats = update_stats(stats, vcf_rec.CHROM, 'REF AND ALT ARE EQUAL')
            ...
            processed_seqids[vcf_rec.CHROM] = True

            if not lr.prev_chrom:
                lr.prev_chrom = vcf_rec.CHROM

            if lr.prev_chrom != vcf_rec.CHROM:
                if lr.prev_chrom not in fasta_file.references:
                    raise G2GValueError("Chromosome {0} not found in {1}".format(lr.prev_chrom, fasta_file.filename))

                # reset
                chrom_length = fasta_file.get_reference_length(lr.prev_chrom)
                last_fragment_size = chrom_length - lr.sums[0] - lr.sums[1]
                end_length = lr.sums[0] + last_fragment_size + lr.sums[2]

                if not lr.header:
                    lr.out.write("# CHAINFILE\n")
                    lr.out.write("# CREATED={0}\n".format(time.strftime("%m-%d-%Y %H:%M:%S")))
                    lr.out.write("# PROGRAM=vcf2chain\n")
                    lr.out.write("# VCF={0}\n".format(vcf_file.file_name))
                    lr.out.write("# FASTA={0}\n".format(fasta_file.filename))
                    lr.header = True

                lr.out.write(CHAIN_STRING.format(CHAIN_STRING,
                            from_chr=lr.prev_chrom, from_length=chrom_length,
                            from_start=0, from_end=chrom_length,
                            to_chr=lr.prev_chrom, to_length=end_length,
                            to_start=0, to_end=lr.sums[0] + last_fragment_size + lr.sums[2], id=lr.prev_chrom))
                lr.out.write("\n")

                for c in lr.chains:
                    lr.out.write("\t".join(map(str, c)))
                    lr.out.write("\n")
                lr.out.write(str(last_fragment_size))
                lr.out.write("\n\n")

                log_stats(stats, lr.prev_chrom)

                # reset variables

                lr.reset()

            LOG.debug("SAMPLE: {0}".format(vcf_rec.SAMPLES[sample_index]))
            LOG.debug("STRAIN('{0}') REF='{1}', ALT_L='{2}', ALT_R='{3}'".format(strain, gt.ref, gt.left, gt.right))

            position = vcf_rec.POS
            ...
            if position < lr.prev_next_ref_pos:
                ...
                stats = update_stats(stats, vcf_rec.CHROM, 'VCF ROLLBACK')
            ...
            if fragment_size < 0:
                ...
                stats = update_stats(stats, vcf_rec.CHROM, 'FRAGMENT SIZE < 0')
            ...
            if fragment_size != 0:
                ...
                lr.chains.append([fragment_size, len(ref_seq), len(alt_seq), shared_bases, ref_str, alt_str, vcf_rec.POS])
                #else:
                #    lr.chains.append([fragment_size, len(ref_seq), len(alt_seq)])
            else:
                #
            ...
            stats = update_stats(stats, vcf_rec.CHROM, 'ACCEPTED')

            LOG.debug(lr.chains[-1])
    ...
    for seqid, processed in processed_seqids.iteritems():
        if not processed:
            LOG.debug("Adding chromosome {0} to chain file".format(seqid))
            for ci, lr in chain_info.iteritems():
                chrom_length = fasta_file.get_reference_length(seqid)

                lr.out.write(CHAIN_STRING.format(CHAIN_STRING,
                    from_chr=seqid, from_length=chrom_length,
                    from_start=0, from_end=chrom_length,
                    to_chr=seqid, to_length=chrom_length,
                    to_start=0, to_end=chrom_length, id=seqid))

                lr.out.write("\n")
                lr.out.write(str(chrom_length))
                lr.out.write("\n\n")
